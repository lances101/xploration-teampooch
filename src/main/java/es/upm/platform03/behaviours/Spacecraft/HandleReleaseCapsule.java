...
public class HandleReleaseCapsule extends CyclicBehaviour {
    ...
    public void action() {
        ...
        try {
            ...
            Location loc = tempLocations.stream().filter(c->c.getKey() == company).findFirst().get().getValue();
            XplorationMap.setCapsuleLocation(company.getCapsule().getCapsule_agent(), loc);
            XplorationMap.updateRoverPosition(company.getCapsule().getRover().getRover_agent(), loc);
